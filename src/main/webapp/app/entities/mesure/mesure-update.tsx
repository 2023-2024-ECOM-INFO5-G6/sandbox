  ...
  return (
    <div>
      <Row className="justify-content-center">
        <Col md="8">
          <h2 id="blogApp.mesure.home.createOrEditLabel" data-cy="MesureCreateUpdateHeading">
            <Translate contentKey="blogApp.mesure.home.createOrEditLabel">Create or edit a Mesure</Translate>
          </h2>
        </Col>
      </Row>
      <Row className="justify-content-center">
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              {!isNew ? (
                <ValidatedField
                  ...
                  readOnly
                  id="mesure-id"
                  label={translate('blogApp.mesure.id')}
                  validate={{ required: true }}
                />
              ) : null}
              <ValidatedField label={translate('blogApp.mesure.type')} id="mesure-type" name="type" data-cy="type" type="select">
                {typeMesureValues.map(typeMesure => (
                  <option value={typeMesure} key={typeMesure}>
                    {translate('blogApp.TypeMesure.' + typeMesure)}
                  </option>
                ))}
              </ValidatedField>
              <ValidatedField label={translate('blogApp.mesure.date')} id="mesure-date" name="date" data-cy="date" type="date" />
              <ValidatedField label={translate('blogApp.mesure.valeur')} id="mesure-valeur" name="valeur" data-cy="valeur" type="text" />
              <ValidatedField
                id="mesure-patient"
                name="patient"
                data-cy="patient"
                label={translate('blogApp.mesure.patient')}
                type="select"
                required
              >
                <option value="" key="0" />
